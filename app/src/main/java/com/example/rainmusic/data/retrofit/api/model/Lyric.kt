...
fun Lyric.parse(): List<LyricLine> {
    val lines = lrc.lyric.split("\n")
        .filter {
            ...
            val ms= it.substring((it.indexOf(".") + 1) until  it.indexOf("]")).toIntOrNull() ?: 0

            LyricLine(
                time = minutes * 60000L + seconds * 1000L + ms,
    ...
    tlyric?.lyric?.split("\n")?.filter {
        ...
        val time = minutes * 60000L + seconds * 1000 + ms
