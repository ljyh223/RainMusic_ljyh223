...
import com.example.rainmusic.data.retrofit.api.model.LikeResult
...
import com.example.rainmusic.data.retrofit.weapi.model.HotPlaylistTag
import com.example.rainmusic.data.retrofit.weapi.model.SubPlaylistResult
import com.example.rainmusic.util.DataState
import com.example.rainmusic.util.encrypt.encryptEApi
import com.example.rainmusic.util.encrypt.encryptWeAPI
import com.example.rainmusic.util.requireOneOf
...
class MusicRepo @Inject constructor(
    ...
    fun getNewSongs(
        ...
        try {
            ...
            Log.d("getNewSongs", e.toString())
            e.printStackTrace()
            emit(DataState.Error(e))
        }
    }

    fun getPersonalFM() = flow {
        emit(DataState.Loading)
        try {
            val result = weApi.personalFM()
            emit(DataState.Success(result))
        } catch (e: Exception) {
            e.printStackTrace()
            emit(DataState.Error(e))
    ...
    fun getPlaylistDetail(
        ...
        try {
            ...
            Log.d("getPlaylistDetail", e.toString())
            e.printStackTrace()
            emit(DataState.Error(e))
        }
    }

    fun getHomePageBlock(
        ...
        try {
            val result =
                weApi.getHomePageBlock(encryptWeAPI(mapOf("refresh" to refresh.toString(), "cursor" to cursor)))
            emit(DataState.Success(result))
        } catch (e: Exception) {
            Log.d("getHomePageBlock", e.toString())
            e.printStackTrace()
            emit(DataState.Error(e))
        }
    ...
    fun getDailyRecommendSongs() = flow {
        ...
        try {
            ...
            Log.d("getDailyRecommendSongs", e.toString())
            e.printStackTrace()
            emit(DataState.Error(e))
        }
    }
